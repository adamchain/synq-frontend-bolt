...
import { getBranchIdByToken, getUserByToken } from './services/index.mjs'
...
app.use(async (req, res, next) => {
  ...
  const token = req.cookies.auth || (authHeader && authHeader.split(' '));

  console.log(token[1])

  if (token) {
    const [user, branchId] = await Promise.all([
      getUserByToken(token[1]),
      getBranchIdByToken(token[1])
    ]);
